...
async function run() {
  try {
    ...
    const ghRepoContext = githubRepos.map(
      (r: { fullName: string; stars: number; forks: number; watchers: number; language?: string }) =>
        `Repo: ${r.fullName}\nStars: ${r.stars}\nForks: ${r.forks}\nWatchers: ${r.watchers}\nLanguage: ${
          r.language || "N/A"
        }`
    );
    ...
    const hnContext = hnStories.map(
      (story: { title: string; score: number; comments: number; author: string }) =>
    ...
    const techNewsContext = techNewsItems.map(
      (item: { title: string; source: string; summary?: string }) =>
        `Title: ${item.title}\nSource: ${item.source}\nSummary: ${
    ...
    const socialContext = socialPosts.map(
      (post: { platform: string; content: string; author: string; hashtags: string[]; score?: number }) =>
    ...
    await summarizeAndUpsert("socialmedia", socialContext, "Social Media");
