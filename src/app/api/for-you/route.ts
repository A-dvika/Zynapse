...
export async function GET(request: Request) {
  try {
    ...
    // Step 3: Fetch recent user activity
    ...
    if (preferences) {
      ...
      userProfileText += `Content Types: ${preferences.contentTypes.join(", ")}. `;
    }
    ...
    if (preferences?.contentTypes?.length) {
      filter.type = { $in: preferences.contentTypes };
    }
